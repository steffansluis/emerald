...
gulp.task('browserify', () => {
  return browserify('dist/emerald.js', {standalone: 'Emerald', sourceType: 'module', debug: true})
    .transform(babelify)
    .bundle()
    .pipe(source('emerald.browser.js'))
    .pipe(gulp.dest('dist'))
});
...
gulp.task('uglify', () => {
  return gulp
    ...
    .pipe(uglify())
    .pipe(rename('emerald.browser.min.js'))
    .pipe(gulp.dest('dist'))
});
...
gulp.task('spec', gulp.series('typescript', done => {
  ...
  gulp.src('spec/*.js')
    ...
    .on('data', data => {
      ...
      if (passing) {
        notifier.notify({title: 'Emerald Test Results', message: 'Tests passed!'});
        return done();
      }
      process.stdout.write("\007");
      notifier.notify({title: 'Emerald Test Results', message: 'Tests failed!'});
      done('failed');
    })
    .pipe(report())
    .pipe(process.stdout);
